...
widget.port.on('showpanel', function(){
	...
	panel.port.on('niepoka_hidemode_get', function(){
		panel.port.emit('niepoka_hidemode_show', isNiePokaSilentMode());
	});
	panel.port.on('niepoka_hidemode_switch', function(){switchNiePokaSilentMode();});
});
...
pageMod.PageMod({
  ...
  onAttach: function(worker) {
    worker.port.emit("performReplace", {list: ss.storage.niepoka_ignorelist, mode: isNiePokaSilentMode()});
...
function isNiePokaSilentMode(){
	if(ss.storage.niepoka_silentMode == undefined)
		ss.storage.niepoka_silentMode = false;
	
	return ss.storage.niepoka_silentMode
}
function switchNiePokaSilentMode(){
	ss.storage.niepoka_silentMode = !ss.storage.niepoka_silentMode;
}